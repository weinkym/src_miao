...
def check_lib_list(lib_list, exists_lib_name_list):
    ...
    for path, name in lib_list:
        ...
        for line in invalid_data_list:
            ...
            #     return False
            print("changed lib={},line={},file={}".format(
                name, line, file_name))
            os.system("install_name_tool -change \"{}\" \"{}\" \"{}\"".format(
                line, file_name, path))
...
def copy_build_lib(build_dp, to_root_dp, lib_subffix_list, need_adjuest=False):
    ...
    if need_adjuest:
        ...
        for fp, name in plugins_list:
            # print('fp={}'.format(fp))
            adjuest_lib(fp)
            os.system("install_name_tool -add_rpath \"{}\" \"{}\"".format(
                "@loader_path/../Frameworks", fp))
            os.system("install_name_tool -add_rpath \"{}\" \"{}\"".format(
                "@loader_path", fp))
            if 'obs-browser' in name:
                os.system("install_name_tool -add_rpath \"{}\" \"{}\"".format(
                    "@loader_path/../Frameworks/Chromium Embedded Framework.framework",
                    fp))
...
# build_dp = "/Users/avc/Documents/TEMP/build-1.x"
build_dp = "/Users/avc/Documents/TEMP/build-obs-2x"
to_root_dp = "/Users/avc/work/ljlive222/vendor/obs/mac"
