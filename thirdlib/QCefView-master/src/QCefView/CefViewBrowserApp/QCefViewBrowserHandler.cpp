...
void QCefViewBrowserHandler::OnDraggableRegionsChanged(CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
...
void QCefViewBrowserHandler::AddLocalDirectoryResourceProvider(const std::string &dir_path,
...
void QCefViewBrowserHandler::AddArchiveResourceProvider(const std::string &archive_path,
    const std::string &url,
...
void QCefViewBrowserHandler::NotifyTakeFocus(bool next)
{
...
void QCefViewBrowserHandler::NotifyDragRegion(const std::vector<CefDraggableRegion> regions)
{
    if(!CefCurrentlyOn(TID_UI))
    {
        // Execute this method on the main thread.
