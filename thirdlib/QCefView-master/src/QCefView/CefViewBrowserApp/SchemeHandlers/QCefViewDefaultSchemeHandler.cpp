...
bool SchemeHandler::Skip(int64 bytes_to_skip, int64 &bytes_skipped, CefRefPtr<CefResourceSkipCallback> callback)
{
    bytes_skipped = -2;
    return false;
}
...
bool SchemeHandler::Read(void *data_out, int bytes_to_read, int &bytes_read, CefRefPtr<CefResourceReadCallback> callback)
{
    ...
    if(offset_ < data_.length())
    {
        ...
        int transfer_size = qMin(bytes_to_read, static_cast<int>(data_.length() - offset_));
